                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <ul className="space-y-1">
                        {(brief.three_things_going_well || []).map((t: string, i: number) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        {(brief.three_risks || []).map((r: string, i: number) => (
                          <li key={i} className="flex gap-2 text-sm text-[#333333]">
                            <span className="text-red-600 font-bold flex-shrink-0">!</span>{r}
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {(brief.entry_points || []).length > 0 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      ...
                      <ul className="space-y-1">
                        {brief.entry_points.map((ep: string, i: number) => (
                          <li key={i} className="flex gap-2 text-sm text-[#333333]">
                            <span className="text-blue-600 flex-shrink-0 font-bold">-</span>{ep}
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div>
                    ...
                    <ol className="space-y-2">
                      {(brief.smart_questions || []).map((q: string, i: number) => (
                  ...
                  {(brief.dont_mention || []).length > 0 && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                      ...
                      <ul className="space-y-1">
                        {brief.dont_mention.map((t: string, i: number) => (
                  ...
                  <div className="bg-gray-100 rounded-lg p-4 border border-gray-200">
                    <p className="text-xs font-bold uppercase tracking-wide text-[#666666] mb-2">Pre-Call Email Draft</p>
                    <p className="text-sm text-[#333333] italic">{brief.one_slide_summary}</p>
                  </div>
